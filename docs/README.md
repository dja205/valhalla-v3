...
User → Odin (global orchestrator, OpenClaw main session)
         ...
         │     ├── Frigg           → System Mapping        (via Copilot CLI)
         ...
         │     ├── Ratatoskr      → QA / Reliability        (via Copilot CLI)
...
```
Request → Mimir (architecture) → Baldr (UX + wireframe) → Frigg (system map)
...
Delivery Pack → Ymir (foundation) → Modi/Magni (implementation)
  → Tyr (code review) → Ratatoskr (QA) → Valkyrie (adversarial testing)
...
24-code-review.md          — Tyr
24b-qa-review.md           — Ratatoskr (versioned: .v1, .v2, etc.)
...
| Frigg | System Mapping | claude-sonnet-4-6 |
...
| Ratatoskr | QA / Reliability Review | claude-sonnet-4-6 |
